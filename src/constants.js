// Shapes
export const BOX = Symbol('box')
export const CIRCLE = Symbol('circle')
export const CYLINDER = Symbol('cylinder')
export const SPHERE = Symbol('sphere')
export const ICOSAHEDRON = Symbol('icosahedron')
export const TORUS = Symbol('torus')
// Texture modes
export const COLOR = Symbol('color')
export const IMAGE = Symbol('image')
// List rearrangement constants
export const TO_PREVIOUS_INDEX = Symbol('to previous index')
export const TO_NEXT_INDEX = Symbol('to next index')
// Drawer tabs
export const SLIDE = Symbol('slide')
export const ROOM = Symbol('room')
export const ADD_ANIMATION = Symbol('add animation')
export const CAPTION = Symbol('caption')
// TransformControls modes and animation styles
export const TRANSLATE = Symbol('translate')
export const ROTATE = Symbol('rotate')
export const SCALE = Symbol('scale')
// Easing functions
export const LINEAR = Symbol('linear')
export const QUAD = Symbol('quad')
// Animation creation steps
export const CHOOSE_ANIMATION_TARGET = 1
export const SELECT_ATTRIBUTE = 2
export const CHOOSE_ANIMATION_DESTINATION = 3
// Animation start time
export const AFTER_PREVIOUS = Symbol('after previous')
export const WITH_PREVIOUS = Symbol('with previous')
